{pkgs, ...}: {
  home.packages = [
    (
      pkgs.writeScriptBin "workspaces" ''
        #!/usr/bin/env bash
        set -euo pipefail

        # Taken from https://Diaoul/dotfiles
        ...
        listen)
          ...
          ${pkgs.socat} -U - "UNIX-CONNECT:$XDG_RUNTIME_DIR/hypr/$HYPRLAND_INSTANCE_SIGNATURE/.socket2.sock" | while read -r line; do handle_event "$line"; done
          ;;
        help)
          cli_help
          ;;
